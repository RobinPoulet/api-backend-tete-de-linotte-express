...
async function* generateSubcategories(categories) {
  for (let i = 0; i < categories.length; i++) {
    ...
    const existingSubcategories = await Subcategory.find({ category: categories[i]._id });
    const subcategories = [];
...
mongoose.connection.once('open', async () => {
  ...
  const subcategories = [];
  for await (const generatedSubcategories of generateSubcategories(categories)) {
